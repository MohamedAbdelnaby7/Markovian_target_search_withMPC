...
class TargetSearchMPC:
    ...
    def simulate_observation(self, sensors, horizon):
        ...
        predicted_target_pos = self.true_position  # Start from the current position
        observations_over_horizon = []  # Store observations at each time step

        if horizon == 0:
            return np.where(np.array(sensors) == self.true_position,
        ...
        for _ in range(horizon):  # Simulate for each future step
            predicted_target_pos = np.random.choice(self.n_states, p=self.transition_matrix[predicted_target_pos])
        ...
        for step in range(steps):
            ...
            for (agent_idx, agent_cell) in enumerate(new_positions):
                if obs[agent_idx] == 1 and self.belief[agent_cell] > detection_threshold:
                    print(f"Target detected by Agent {agent_idx} at step {step}, cell {divmod(self.true_position, 50)}.")
